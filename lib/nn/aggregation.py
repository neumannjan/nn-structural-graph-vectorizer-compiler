...
class ReshapeAndAggregate(torch.nn.Module, ReshapeAggregateModuleLike):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.profiler.record_function('RESHAPE_AGG__RESHAPE'):
            x = self.reshape(x)
        with torch.profiler.record_function('RESHAPE_AGG__AGGREGATE'):
            x = self.aggregate(x)
        return x
...
class ScatterReduceAggregation(torch.nn.Module, ReshapeAggregateModuleLike):
    ...
    def forward(self, inp: torch.Tensor) -> torch.Tensor:
        with torch.profiler.record_function('AGGREGATE__SCATTER_REDUCE'):
