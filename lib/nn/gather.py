...
class GatherAndRepeatNonOptimal(torch.nn.Module, GatherModuleLike):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.profiler.record_function('GATHER_AND_REPEAT__GATHER'):
            x = self.gather(x)
        with torch.profiler.record_function('GATHER_AND_REPEAT__REPEAT'):
            x = self.repeat(x)
        return x
...
class LayersGatherConcat(torch.nn.Module, LayerGatherModuleLike):
    ...
    def forward(self, layer_values: dict[int, torch.Tensor]):
        with torch.profiler.record_function('LAYERS_GATHER_CONCAT__RUN_INDIVIDUAL_LAYERS'):
            xs = [self.layer_gathers[str(layer)](layer_values[layer]) for layer in self.layers]

        with torch.profiler.record_function('LAYERS_GATHER_CONCAT__BROADCAST_SHAPES'):
        ...
        with torch.profiler.record_function('LAYERS_GATHER_CONCAT__EXPAND'):
            xs = [t.expand(*shape_hull) for t in xs]

        with torch.profiler.record_function('LAYERS_GATHER_CONCAT__CONCAT'):
...
class MultiLayerGather(torch.nn.Module, LayerGatherModuleLike):
    ...
    def forward(self, layer_values: dict[int, torch.Tensor]) -> torch.Tensor:
        with torch.profiler.record_function('GATHER_MULTI__GATHER_LAYERS_UNIQUE'):
            x = self.multi_layer_set_gather(layer_values)
        with torch.profiler.record_function('GATHER_MULTI__FINAL_GATHER'):
...
class GatherAndReshape(torch.nn.Module, GatherModuleLike):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.profiler.record_function('GATHER_RESHAPE__GATHER'):
            x = self.gather(x)
        with torch.profiler.record_function('GATHER_RESHAPE__RESHAPE'):
            x = self.reshape(x)
        return x
