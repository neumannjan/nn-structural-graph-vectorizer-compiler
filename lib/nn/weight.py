...
class StackWeights(torch.nn.Module, WeightModuleLike):
    ...
    def forward(self) -> torch.Tensor:
        with torch.profiler.record_function('WEIGHTS_CONCAT'):
...
class ExpandWeight(torch.nn.Module, WeightModuleLike):
    ...
    def forward(self) -> torch.Tensor:
        with torch.profiler.record_function('WEIGHT_EXPAND'):
...
class StackWeightModules(torch.nn.Module, WeightModuleLike):
    ...
    def forward(self) -> torch.Tensor:
        with torch.profiler.record_function('STACK_WEIGHT_MODULES__CALL'):
            modules = [m() for m in self.the_modules]
        with torch.profiler.record_function('STACK_WEIGHT_MODULES__CONCAT'):
