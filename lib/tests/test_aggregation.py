...
SAMPLE1 = MockJavaNeuron(_n(), 16, [
    MockJavaNeuron(_n(), 13, [
        MockJavaNeuron(_n(), 12, [])
        for _ in range(4)
    ])
    for _ in range(3)
])

SAMPLE2 = MockJavaNeuron(_n(), 16, [
    MockJavaNeuron(_n(), 13, [
        MockJavaNeuron(_n(), 12, [])
        for _ in range(4)
    ]),
...
def test_same_no_of_inputs():
    ...
    layer = AggregationLayer(layer_neurons=SAMPLE1.getInputs(), neuron_ordinals=ordinals, aggregation='sum')
...
def test_variable_no_of_inputs():
    ...
    _, ordinals = compute_neuron_ordinals(LAYERS, network, Settings())
    layer = AggregationLayer(layer_neurons=SAMPLE2.getInputs(), neuron_ordinals=ordinals, aggregation='sum')
